...
from hmotiongpt.data.collators import AlignmentCollator
...
def run_alignment(config: Dict[str, Any]) -> Path:
    ...
    tokenizer, imu_llm, imu_token_ids = create_tokenizer_and_model(config, freeze_llm=True)
    ...
    for epoch in range(int(config["train"]["num_epochs"])):
        ...
        for batch_idx, batch in enumerate(dataloader, start=1):
            ...
            if should_step:
                grad_norm = torch.nn.utils.clip_grad_norm_(projector.parameters(), max_grad_norm).item()
                optimizer.step()
                scheduler.step()
                optimizer.zero_grad()
            else:
            ...
            if batch_idx % int(config["train"].get("log_every", 10)) == 0:
                print(
                    f"[alignment][epoch={epoch}][step={batch_idx}] "
        ...
        print(f"[alignment][epoch={epoch}] avg_loss={avg_loss:.4f}")
